# 👻 Phantom V2 👻
## ⚠️ THIS VERSION HAS BEEN ARCHIVED. YOU CAN FIND V3 IN MY REPOS. ⚠️


## WORKS ON APPROXIMATELY 81.7% OF WEBSITES!

### What is Phantom?
Phantom is an easy bypass for the extension known as Securly.
### What can it do?
Phantom bypasses Securly in 2 ways. 1st it lets you visit websites that are blocked by the Securly extension, and 2nd it doesn't show up on the Securly monitoring extension.
### Why?
Securly has terrible security, and I wanted to take advantage of that and spread awareness.
### How can I use this?
Download the .html file and open it... that's literally it lol
### COMMOM ERRORS!!!
A common error is the Refused to Connect page. This occurs because Phantom uses iframes, and some websites use iframe restrictions. I am working on a proxy to bypass this 🫡

(IF YOU ARE AN ADMIN FROM MY SCHOOL DISTRICT AND YOU SOMEHOW GOT AHOLD OF THIS PROJECT, I WOULD LIKE TO SPECIFY THAT THIS HAS ALREADY BEEN PATCHED BY YOU GUYS VIA BLOCKING JS FROM RUNNING IN HTML FILES, I AM NOT ACTIVLY USING THIS, THIS IS AN ARCHIVE FOR OTHERS TO CHECK OUT.)
